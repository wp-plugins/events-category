...
var eventscategory_id = <?php echo get_option('eventscategory_ID') ?>;
var eventscategory_all_id_lookup = {
	<?php
	#get_all_category_ids();
	$evt_cats = array();
	#print_r(get_categories('hide_empty=0&child_of=' . get_option('eventscategory_ID')));
	foreach(get_categories('hide_empty=0&child_of=' . get_option('eventscategory_ID')) as $evt_cat){
		$evt_cats[] = $evt_cat->cat_ID;
	}
	echo join(':true,', $evt_cats) . ':true';
	?>
};
eventscategory_all_id_lookup[eventscategory_id] = true;
</script>
<div class='datetime'>
	<label for="eventscategory_dstart" style="<?php _e('display:inline-block; width:11ex; text-align:right; padding-right:5px;', 'events-category') ?>"><?php _e('Time: ', 'events-category') ?>
	</label><input type="date" id="eventscategory_dstart" name="eventscategory_dstart" value="" required="required" />
	<input type="time" id="eventscategory_tstart" name="eventscategory_tstart" value="" required="required" />
...
<div class='location'>
	...
	$fieldsLocale = array(
		'fn_org'          => array(
			...
			'label-style' => __('display:inline-block; width:11ex; text-align:right; padding-right:5px;', 'events-category')
		),
		//'post-office-box',
